...
The file to store the database in. The directory where the database is to be
stored must exist. Musium stores persistent state in a SQLite database at this
location. It contains the listens database, cached tag metadata, and cover art.
Updating the listens database causes disk activity for every track, so it is
recommended to keep the data path on a silent storage medium. See also [the
section on disks](disks.md) for more details.
